...
CONFIG = None
with open("D:\Hackathon\Daksh'24\ChatBot\Final\config.json") as f:
    CONFIG = json.load(f)
...
# class TokenizerEmbeddings(Embeddings):
...
embeddings = AutoTokenizer.from_pretrained(
    "Intel/neural-chat-7b-v3-3", trust_remote_code=True, model_kwargs=model_kwargs, encode_kwargs=encode_kwargs)
# embeddings = TokenizerEmbeddings(tokenizer)
...
def load_llm():
    ...
    llm = AutoModelForCausalLM.from_pretrained(
        "Intel/neural-chat-7b-v3-3", trust_remote_code=True, config=conf)
...
def search_product(prompt, user_allergies):
    ...
    print("[$$] Time Taken:", ed-st, "secs")
    print("[9] Outside search_product")
    return res
